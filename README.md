...
- `StationSeedRegistry` — uma seed por `(repetition, cs_amount)`. Cada
  approach com seed sorteia a seleção de estações de forma independente
  pra cada tamanho — não há relação entre a seleção de 9 e a de 16
  estações.
...
- **`random_strategy.py`** — sorteia trincas de `<connection>`, valida
  ciclo alcançável no grafo, aceita a lane do meio. Sem crescimento
  incremental (ver seção 5).
- **`pseudorandom_strategy.py`** — usa a malha de quadrantes do mesmo
  ...
  max_vehicles_per_cs`). Se os arquivos de malha ainda não existirem,
  chama `quadrants.generate_all_quadrants()` sozinha (via
  `quadrants_check.py`) antes de tentar ler. Sem crescimento incremental.
- **`greedy_strategy.py`** — determinística, sem seed: associa as lanes
  mais visitadas (`mostVisited.xml`, ordenadas por `count` decrescente) à
  primeira quadrante ainda vazia que as contém. Também aciona
  `quadrants_check.py` sozinha se precisar. Um quadrante sem nenhuma lane
  visitada simplesmente fica sem estação (não é erro).
- **`greedy_voronoi_strategy.py`** — sorteia pontos aleatórios, monta um
  diagrama de Voronoi (desenhado manualmente com matplotlib, sem depender
  de `scipy.spatial.voronoi_plot_2d`, que tem um bug conhecido em algumas
  versões), associa lanes mais visitadas às regiões. Uma região sem lane
  visitada tocando nela fica sem estação (não é erro).
- **`evolution.py`** — cache em disco da seleção de estações por
  `(approach, repetition, cs_amount)`, usado pelos 3 approaches com seed
  pra não recalcular à toa entre `percentage`/`minutes` diferentes (ver
  seção 5).
...
### 5.3 Cada `cs_amount` sorteia de forma independente

Os 3 approaches com seed (`random`, `pseudorandom`, `greedyvoronoi`) hoje
sorteiam a seleção de estações **do zero, de forma independente, pra cada
`cs_amount`** — não existe relação entre a seleção de 9 estações e a de 16,
25, 36 ou 49: cada tamanho tem sua própria seed (`(repetition, cs_amount)`)
e seu próprio resultado, sem nenhuma garantia de que as posições de um
tamanho apareçam dentro de outro.

> **Nota histórica:** durante a refatoração, chegamos a implementar um
> mecanismo de **crescimento incremental** (cada seed representando "uma
> evolução da cidade", onde 9→16→25→36→49 preservava as posições
> anteriores e só adicionava novas). Foi implementado e testado nos 3
> approaches em algum momento, mas foi **revertido nos 3**, um de cada vez,
> por decisão explícita — a intenção final do experimento é que cada
> `cs_amount` seja uma amostra independente, não uma sequência evolutiva.
> `station_strategies/evolution.py` ainda existe, mas hoje é só um cache
> em disco por `(approach, repetition, cs_amount)` (evita recalcular à toa
> entre `percentage`/`minutes` diferentes) — não tem mais nenhuma lógica
> de cascata/dependência entre tiers.

**`greedy`** fica fora dessa discussão inteira: não usa seed nenhuma (é
determinístico), então nem se aplica o conceito de "independente vs.
incremental" — ele sempre recalcula, mas sempre dá o mesmo resultado pra
um dado `cs_amount`.
...
```
output/<approach>/
├── seeds_stations_registry.json      # {"1rep_9cs": 3271958341, "1rep_16cs": ..., ...}
├── seeds_trips_registry.json         # {"20pct_1rep": ..., ...}
├── stations_evolution/               # nome histórico -- hoje é só um cache, sem cadeia entre tiers
│   └── seed_<repetition>/
│       ├── 9stations.xml
│       ├── 16stations.xml
│       ├── 25stations.xml
│       ├── 36stations.xml
│       └── 49stations.xml            # cada arquivo é independente dos outros
├── jobs/                             # manifestos de cada SimJob, um .json por combinação (reexecução manual)
└── <minutes>min/
    └── <percentage>percentage<cs>cs/
        ...
        ├── Nbattery<percentage>percentage<cs>cs.xml       # --battery-output do SUMO
        ├── logN<percentage>percentage<cs>cs.xml           # log do SUMO
        ├── reports/REPORT-Nfile-Ttime-Vvehicles-Mmax...
        ├── selectedLanes/Nchargingstations.xml            # cópia da seleção cacheada, para este job
...
seu `output.zip`), com o acréscimo do nível `<minutes>min/` (seção 5.2).
`OUTPUT_DIR` fica em `TCC/output` (irmã de `input/` e `MD/`), não mais
dentro de `MD/output/`.
