...
Uma pasta <minutes>min/ só é considerada "completa" -- e só então gera
CSV -- quando TODAS as combinações esperadas de
(cs_amount x percentage x repetition), segundo a grade definida em
config.py (STATIONS_AMOUNTS x PERCENTAGES x REPETITIONS), têm log com
resumo final completo. Pastas parciais são puladas (nada é escrito),
com uma mensagem indicando quantas combinações já estão prontas.
...
def expected_combos() -> set[tuple[int, int, int]]:
    """(percentage, cs_amount, repetition) esperados para qualquer
    (approach, minutes), segundo a grade definida em config.py."""
    return {
        (percentage, cs_amount, repetition)
...
def missing_combos(rows: list[dict]) -> set[tuple[int, int, int]]:
    """Compara o que foi encontrado (com resumo completo) em `rows` contra
    a grade esperada, e devolve o que ainda falta. Não faz I/O extra --
    reaproveita as rows que collect_rows() já leu do disco (os logs podem
    ser arquivos grandes, então evitamos ler cada um duas vezes)."""
    found = {(r["percentage"], r["cs_amount"], r["repetition"]) for r in rows}
    return expected_combos() - found
...
def main() -> None:
    ...
    total_esperado = len(expected_combos())
    ...
    for approach in args.approach:
        ...
        for minutes in minutes_dirs:
            ...
            faltando = missing_combos(rows)

            if faltando:
                prontas = total_esperado - len(faltando)
                print(
                    f"  {minutes}min: incompleta ({prontas}/{total_esperado} "
                    f"combinações prontas) -- CSV não gerado"
                )
                continue

            out_path = write_csv(approach, minutes, rows)
            print(f"  {minutes}min: completa -- {len(rows)} job(s) -> {out_path}")
