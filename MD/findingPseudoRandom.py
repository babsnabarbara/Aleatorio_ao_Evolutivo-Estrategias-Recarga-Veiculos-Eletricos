...
def sortCarsAndTrips(numberOfCars, fromRoute):
    ...
    inputFolder = "../input/"
    outputFolder = "../output/pseudorandom/"+str(numberOfCars)+"/"
    outputNet = "rotas"
    netInCgFile = "cologne"
...
def makeCfgs(percentage, newFolder, amountOfCars):
    ...
    for i in range(1,6):
        ...
        outputFolder = "../output/pseudorandom/" + newFolder + "/"
...
def findAndSaveValidChargingPoints(netfile, args):
    print("ARGS CS AMOUNT : "+ args.csAmount)
    if int(args.csAmount) != 0:
        ...
        quadrantsFile = "../input/lanesInEachQuadrant/"+str(args.csAmount)+"quadrants.xml"
        ...
        for variation in range(1, 6):
                ...
                while (not valid):
                    ...
                    if (trying == 3):
                        ...
                        if chargingPoint not in chargingPoints:
                            if (traci.lane.getLength(chargingPoint) / traci.vehicletype.getLength("soulEV65") ) >= int(args.maxVehiclesPerCS):
                                valid = True
            ...
            functions.createSelectedCSFiles(currentChosenPoints,variation, args.folder)
            #chargingEachQuadrant[variation]['chargingPoints'] = currentChosenPoints
    else:
        emptySet = set()
        for variation in range(1, 6):
            functions.createSelectedCSFiles(emptySet, variation, args.folder)


def makeAdds(chargingEachQuadrant, newFolder):
    for i, info in chargingEachQuadrant.items():
        outputNet = "cologne"
        outputFolder = "../output/pseudorandom/"
...
def sortearCarrosPorPercentual(filename, porcentagem, numberFile, amountOfCars, newFolder):
    ...
    with open(filename, 'r', encoding='utf-8') as file_origem:
        ...
        nome_arquivo_sorteadas = "../output/pseudorandom/"+ newFolder+"/sortedCars" + str(numberFile) + ".xml"
        ...
        nome_arquivo_sem_sorteadas = '../output/pseudorandom/'+newFolder+'/cologne6to8-' + str(numberFile) + ".trips.xml"
...
def main ():
    ...
    chargingEachQuadrant = findAndSaveValidChargingPoints('../input/cologne.net.xml', args)
